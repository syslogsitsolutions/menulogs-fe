  ...
  private static generateBillHTML(bill: BillData): string {
    ...
    return `
      <div style="text-align: center; margin-bottom: 20px;">
        ${business.logo ? `<img src="${business.logo}" alt="${business.name}" style="max-width: 150px; margin-bottom: 10px;">` : ''}
        <h1 style="margin: 0; font-size: 24px; font-weight: bold;">${business.name}</h1>
        <p style="margin: 5px 0; font-size: 14px;">${location.name}</p>
        <p style="margin: 2px 0; font-size: 12px;">${location.address}</p>
        <p style="margin: 2px 0; font-size: 12px;">${location.city}, ${location.state} ${location.zipCode}</p>
        <p style="margin: 2px 0; font-size: 12px;">Phone: ${location.phone}</p>
        ${location.email ? `<p style="margin: 2px 0; font-size: 12px;">Email: ${location.email}</p>` : ''}
      </div>
      
      <hr style="border: none; border-top: 2px solid #000; margin: 20px 0;">
      
      <div style="margin-bottom: 20px;">
        <h2 style="margin: 0 0 10px 0; font-size: 20px;">Invoice #${order.orderNumber}</h2>
        <p style="margin: 5px 0;"><strong>Date:</strong> ${date}</p>
        ${completedDate ? `<p style="margin: 5px 0;"><strong>Completed:</strong> ${completedDate}</p>` : ''}
        ${order.table ? `<p style="margin: 5px 0;"><strong>Table:</strong> ${order.table.number}${order.table.name ? ` (${order.table.name})` : ''}</p>` : ''}
        <p style="margin: 5px 0;"><strong>Type:</strong> ${order.type.replace('_', ' ')}</p>
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <thead>
            <tr style="background-color: #f5f5f5;">
              <th style="text-align: left; border-bottom: 2px solid #000; padding: 10px;">Item</th>
              <th style="text-align: center; border-bottom: 2px solid #000; padding: 10px;">Qty</th>
              <th style="text-align: right; border-bottom: 2px solid #000; padding: 10px;">Price</th>
              <th style="text-align: right; border-bottom: 2px solid #000; padding: 10px;">Total</th>
            </tr>
          </thead>
          <tbody>
            ${order.items
              ...
              <tr>
                <td style="padding: 8px; border-bottom: 1px solid #ddd;">
                  <strong>${item.menuItemName}</strong>
                  ${item.notes ? `<br><span style="font-size: 11px; color: #666; font-style: italic;">${item.notes}</span>` : ''}
                </td>
                <td style="text-align: center; padding: 8px; border-bottom: 1px solid #ddd;">${item.quantity}</td>
                <td style="text-align: right; padding: 8px; border-bottom: 1px solid #ddd;">$${item.unitPrice.toFixed(2)}</td>
                <td style="text-align: right; padding: 8px; border-bottom: 1px solid #ddd;">$${item.totalPrice.toFixed(2)}</td>
              </tr>
            `
              )
              .join('')}
      ...
      <div style="margin-top: 20px; margin-left: auto; width: 300px;">
        <table style="width: 100%; border-collapse: collapse;">
          ...
          ${order.taxRate > 0 ? `
          <tr>
            <td style="padding: 5px; text-align: right;">Tax (${order.taxRate}%):</td>
            <td style="padding: 5px; text-align: right;">$${order.taxAmount.toFixed(2)}</td>
          </tr>
          ` : ''}
          ${order.discountAmount > 0 ? `
          <tr>
            <td style="padding: 5px; text-align: right;">Discount:</td>
            <td style="padding: 5px; text-align: right;">-$${order.discountAmount.toFixed(2)}</td>
          </tr>
          ` : ''}
          ${order.tipAmount > 0 ? `
          <tr>
            <td style="padding: 5px; text-align: right;">Tip:</td>
            <td style="padding: 5px; text-align: right;">$${order.tipAmount.toFixed(2)}</td>
          </tr>
          ` : ''}
          <tr style="border-top: 2px solid #000;">
            <td style="padding: 10px 5px; text-align: right;"><strong>Total:</strong></td>
            <td style="padding: 10px 5px; text-align: right;"><strong>$${order.totalAmount.toFixed(2)}</strong></td>
          </tr>
        </table>
      </div>
        ...
        <table style="width: 100%; border-collapse: collapse;">
          ${payments
            .map(
              (payment) => `
            <tr>
              <td style="padding: 5px;">${payment.method.toUpperCase()}</td>
              <td style="padding: 5px; text-align: right;">$${payment.amount.toFixed(2)}</td>
              ${payment.transactionId ? `<td style="padding: 5px; font-size: 10px; color: #666;">Txn: ${payment.transactionId}</td>` : ''}
            </tr>
          `
            )
            .join('')}
      ...
      <div style="text-align: center; margin-top: 30px;">
        <p style="margin: 10px 0; font-size: 14px;"><strong>Thank you for your visit!</strong></p>
        ${order.notes ? `<p style="margin: 10px 0; font-size: 12px; color: #666;">${order.notes}</p>` : ''}
      </div>
    `;
  }
}
